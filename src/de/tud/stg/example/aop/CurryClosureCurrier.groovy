...
aspect(name:"CurryClosureCurrier",deployed:false) {
	before (service_call("getRate(.)*") & if_pcd { external }) {
		...
		println "ASPECT(${thisAspect.name}): \t >Receiving closure over transferCurryClosure"
		transferClosure = transferCurryClosure.curry(4)
