 ...
 */
public class MetaCallPCD extends PrimitivePCD {
	
	private Pattern compiledSignatureRegExp;
	
	public MetaCallPCD(String signatureRegExpr) {
		super("meta_call("+signatureRegExpr+")");
	...
	public boolean matchUncached(JoinPoint jp) {
		if (jp instanceof MetaCallJoinPoint){
			return compiledSignatureRegExp.matcher(((MetaCallJoinPoint)jp).methodName).matches();
		}else{
			return false;
		}
	}
	...
	public Pointcut partialEvalUncached(StaticJoinPoint staticJoinPoint) {
		if (MetaCallJoinPoint.class.isAssignableFrom(staticJoinPoint.getCorrespondingNonStaticJoinPointType())){
