...
public class FibonacciGroovyBenchmarkRunner extends MOPBenchmarkRunner {
	private int size;
	private FibonacciGroovy fibonacci;
	
	public FibonacciGroovyBenchmarkRunner(int size, BenchmarkMOPMode mopMode, BenchmarkAspectMode aspectMode, boolean enableInterTypeDeclarationCache, boolean enablePartialEvaluation, boolean debug) {
		super(mopMode, aspectMode, enablePartialEvaluation, enableInterTypeDeclarationCache, debug);
		this.size = size;
		fibonacci = new FibonacciGroovy();
	}

	@Override
	protected StructureDesignator getBenchmarkSpecificClassDesignator() {
		return StructuralPointcutDSL.eval{is_type(FibonacciGroovy)}
	}

	@Override
	protected void runBenchmark() {
		if(debug) print "FibonacciGroovy: Calculating fib("+size+") = "
