...
public class AspectGenerator {
	 ...
	 * @params		testObject			Object under test.
	 */
	def aspectTemplate = {int numberOfAspects, String aspectBasicName, TestObject testObject, ArrayList<Integer> priorityList ->
		...
		println "Generating aspect: numberOfAspects=$numberOfAspects, aspectBasicName=$aspectBasicName, testObject=$testObject, priorityList=$priorityList"
		
		for ( i in 1..numberOfAspects ) {
			def aspectName = "$aspectBasicName$i"
			print "Initializing aspect $aspectName "
			
			assert priorityList.size()== numberOfAspects
			def p = priorityList[i-1];
			println "with priority $p"
